...
public class FarmFruitTree extends WorldGenerator
{
    ...
    public FarmFruitTree()
    {
    }
    
    public boolean generate(World world, Random random, int i, int j, int k, int block, int meta){
    ...
    public boolean generate(World world, Random random, int i, int j, int k)
    {
        ...
        for(int k1 = (j - 3) + l; k1 <= j + l; k1++)
        {
            ...
            for(int k3 = i - i3; k3 <= i + i3; k3++)
            {
                ...
                for(int i4 = k - i3; i4 <= k + i3; i4++)
                {
                    ...
                    {
                        if(world.getBlockId(k3, k1, i4) == 17)
                        {
                            continue;
                        }
                        if(world.getBlockId(k3, k1 - 1, i4) == FruitBlock)
                        {
                            world.setBlock(k3, k1, i4, Blocks.leaves.blockID, leavesMeta, 2);
                        } else
                        {
                            world.setBlock(k3, k1, i4, FruitBlock);
                            world.setBlock(k3, k1 + 1, i4, Blocks.leaves.blockID, leavesMeta, 2);
                        }
                    } else
                    {
                        world.setBlock(k3, k1, i4, Blocks.leaves.blockID, leavesMeta, 2);
        ...
        for(int l1 = 0; l1 < l; l1++)
        {
            ...
            if(k2 == 0 || k2 == Blocks.leaves.blockID)
            {
                world.setBlock(i, j + l1, k, 17);
            }
        }
