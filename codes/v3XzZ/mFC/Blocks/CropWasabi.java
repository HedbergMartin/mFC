...
public class CropWasabi extends CropTemplate
{
	protected int renderType;
	
	public CropWasabi(int i)
    {
        super(i);
    ...
    public void dropBlockAsItemWithChance(World world, int i, int j, int k, int l, float f, int i1)
    {
        ...
        for(int i2 = 0; i2 < 4; i2++)
        {
            if(world.rand.nextInt(10) <= l)
            {
                ...
                EntityItem entityitem = new EntityItem(world, (float)i + f2, (float)j + f3, (float)k + f4, new ItemStack(Items.wasabiSeeds));
    ...
    public int idDropped(int i, Random random, int j)
    {
        if(i == 2)
        {
            return Items.wasabiRoot.itemID;
        } else
        {
            return -1;
        }
	...
	public int idPicked(World par1World, int par2, int par3, int par4) {
		return Items.wasabiSeeds.itemID;
	}
}